...
#define LED_PIN A2 // LED pin

// ================================================================
// ===                  Software I2C setup                      ===
// ================================================================
//The slave I2C address should be changed manually here.
//此处需手动修改从机软件I2C地址
#define soft_I2C_SLAVE_ADDRESS 3
byte i2c_address = soft_I2C_SLAVE_ADDRESS;

#define IS_ADDR_EVEN (((soft_I2C_SLAVE_ADDRESS) & 1) == 0)

#define bufferSize 17   // 17 bytes per iic sending 
...
byte imu_data[bufferSize] = "no data"; // must be initialized  
union FloatToBytes {
    float value = 6.66666;
    byte bytes[4];
};
FloatToBytes w, x, y ,z;
...
// data sending and receiving functions in software I2C
extern "C" 
{
...
void ReadIMUbyDMP()
{
    ...
    else if (mpuIntStatus & 0x02) 
    {
        ...
        // convert and merge the quaternion into the imu_data[](char)
        dataConversion(q.w, q.x, q.y, q.z);  
    }
  }
...
void dataConversion(float qw, float qx, float qy, float qz)
{
  ...
  memcpy(imu_data, &i2c_address, 1);
  memcpy(imu_data + 1, w.bytes, 4);
  memcpy(imu_data + 5, x.bytes, 4);
  memcpy(imu_data + 9, y.bytes, 4);
  memcpy(imu_data + 13, z.bytes, 4);
}
